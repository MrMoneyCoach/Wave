...
function writeClaudeMd(dir: string, projects: Project[]): void {
  ...
  const body = [
    ...
    "",
    "**The user uses Microsoft Outlook for email, and Word/Excel for documents/spreadsheets.** Never default to Apple Mail, Pages, or Numbers.",
    "",
    "### Web / files / apps",
    ...
    "- `linkedin_jobs_search({query, location?, remote?, experience?, postedWithin?, easyApply?})` — full filter set. `remote`: onsite/remote/hybrid. `experience`: internship/entry/associate/mid/senior/director/executive. `postedWithin`: 24h/week/month.",
    "- `linkedin_people_search({query, title?, company?, location?})` — find individuals.",
    "- `linkedin_company_search({query})`.",
    "- `linkedin_goto({section})` — feed / messaging / notifications / mynetwork / jobs / profile / saved_jobs / my_posts.",
    "- `linkedin_compose_post()` — opens the post composer; pair with `paste_text` to drop in a drafted post.",
    ...
    "- *\"Email Sam about Friday's lunch\"* → write the body yourself, then `outlook_compose({to: ['sam@…'], subject: 'Lunch Friday', body: '…'})`. Leave it for review.",
