...
function writeClaudeMd(dir: string, projects: Project[]): void {
  ...
  const body = [
    ...
    "- `open_site_search(site, query)` — opens a search URL on a well-known site (LinkedIn, Google, YouTube, Amazon, Wikipedia, Reddit, X/Twitter, GitHub) in the user's default browser. Because it opens in the user's real browser, their login (e.g. LinkedIn) is reused — no auth handling needed.",
    "- `find_files(query, onlyIn?, limit?)` — Spotlight search. Returns absolute paths.",
    "- `reveal_file(path)` / `open_file(path)` / `open_app(name)`.",
    "",
    "**Calendar**",
    "- `calendar_create_event({title, start, end?, notes?, location?, calendar?})` — ISO 8601 dates.",
    "- `calendar_list_events({from?, to?, calendar?})` — defaults to today.",
    "",
    "**Reminders**",
    "- `reminder_create({title, due?, list?, notes?})`.",
    "- `reminder_list({list?, limit?})` — returns incomplete reminders.",
    "",
    "**Notes**",
    "- `note_create({title, body, folder?})` — body may contain basic HTML.",
    "- `note_append({title, text, folder?})` — appends to an existing note, or creates it.",
    "",
    "**Clipboard / dictation**",
    "- `clipboard_get()` / `clipboard_set({text})`.",
    "- `paste_text({text})` — switches to the previously-focused app and pastes. Use when the user says things like *'type this into the email I'm writing'* or *'paste this into my document'*. Requires Accessibility permission once.",
    "",
    "### Worked examples",
    "",
    "- *\"Search LinkedIn for Swift Engineer roles in London\"* → `open_site_search(site='linkedin_jobs', query='Swift Engineer London')`.",
    ...
    "- *\"Add a reminder to call mum tomorrow at 6pm\"* → `reminder_create({title: 'Call mum', due: '<ISO tomorrow 18:00>'})`. Confirm briefly.",
    "- *\"Put lunch with Sam in my calendar for Friday 1pm\"* → `calendar_create_event({title: 'Lunch with Sam', start: '<ISO Friday 13:00>'})`.",
    "- *\"Make a note of today's standup: <notes>\"* → `note_create({title: 'Standup <date>', body: '<html notes>'})`.",
    "- *\"Type this into the email I'm writing: <text>\"* → `paste_text({text})`. Tell the user it was pasted.",
    "",
    "Dates: when the user says things like *'tomorrow at 6pm'*, convert them to ISO 8601 yourself. Assume the user's local timezone.",
    "",
    "For general research/reading without opening a browser tab, prefer WebSearch/WebFetch. Use the open_* tools when the user wants to **see** something.",
    "",
    "## Style",
    "",
