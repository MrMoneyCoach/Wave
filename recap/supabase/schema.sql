...
-- --------------------------------------------------------------------------
-- Phase 7: sharing, comments, integrations.
-- --------------------------------------------------------------------------
...
create policy "meeting_shares: owner manage" on public.meeting_shares
  for all
  using (exists (select 1 from public.meetings m where m.id = meeting_shares.meeting_id and m.owner_id = auth.uid()))
  with check (exists (select 1 from public.meetings m where m.id = meeting_shares.meeting_id and m.owner_id = auth.uid()));
...
create policy "meeting_shares: shared user reads own" on public.meeting_shares
  for select using (lower(shared_with_email) = lower(coalesce(auth.jwt() ->> 'email', '')));
